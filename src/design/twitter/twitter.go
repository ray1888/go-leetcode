...
/*
	总体思路：
	1. 用户关注使用map[int64][]int64来进行存储做增删改查
	2. 用户自己维护一个自己的发推列表

	获取getTwitte的时候有两种处理方式：
	1. 在线处理，把此用户及其所有关注的用户发的推进行归并排序，每次归并取前十个结果，这样的时间复杂度为o(10n)
	2. 离线处理，在每次其关注用户及自己发推或者follow，unfollow别人的时候进行离线的计算，维护一个大小为10的堆，这样的话，获取的时候就是o(1)
	   但是离线计算时候逻辑其实也是一样，因此提升了接口的响应速率，并没有减少运算的时间复杂度。
*/
